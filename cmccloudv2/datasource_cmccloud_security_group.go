...
func dataSourceSecurityGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	if securityGroupId := d.Get("security_group_id").(string); securityGroupId != "" {
		ecsgroup, err := client.SecurityGroup.Get(securityGroupId)
		if err != nil {
			if errors.Is(err, gocmcapiv2.ErrNotFound) {
				d.SetId("")
				return fmt.Errorf("unable to retrieve security group [%s]: %s", securityGroupId, err)
			}
		}
