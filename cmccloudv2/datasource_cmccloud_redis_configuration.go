...
func dataSourceRedisConfigurationRead(d *schema.ResourceData, meta interface{}) error {
	...
	if configurationId := d.Get("configuration_id").(string); configurationId != "" {
		configuration, err := client.RedisConfiguration.Get(configurationId)
		if err != nil {
			if errors.Is(err, gocmcapiv2.ErrNotFound) {
				d.SetId("")
				return fmt.Errorf("unable to retrieve redis configuration [%s]: %s", configurationId, err)
			}
		}
...
func dataSourceComputeRedisConfigurationAttributes(d *schema.ResourceData, configuration gocmcapiv2.RedisConfiguration) error {
	...
	if configuration.ID2 != "" {
		errs = append(errs, d.Set("configuration_id", configuration.ID2))
		d.SetId(configuration.ID2)
	} else {
		d.SetId(configuration.ID)
		errs = append(errs, d.Set("configuration_id", configuration.ID))
	}
