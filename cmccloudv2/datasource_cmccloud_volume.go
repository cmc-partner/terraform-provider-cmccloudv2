...
func dataSourceVolumeRead(d *schema.ResourceData, meta interface{}) error {
	...
	if volumeId := d.Get("volume_id").(string); volumeId != "" {
		volume, err := client.Volume.Get(volumeId)
		if err != nil {
			if errors.Is(err, gocmcapiv2.ErrNotFound) {
				d.SetId("")
				return fmt.Errorf("unable to retrieve volume [%s]: %s", volumeId, err)
			}
		}
...
func dataSourceComputeVolumeAttributes(d *schema.ResourceData, volume gocmcapiv2.Volume) error {
	...
	errs = append(errs, d.Set("server_id", serverId))
